...
class ImageService:
    ...
    def _choose_best_upright(self, img_bgr: np.ndarray) -> Tuple[np.ndarray, int, dict]:
        ...
        try:
            ...
            def downscale(
                bgr: np.ndarray, limit: int = 2400
            ) -> np.ndarray:  # Increased from 1200 for better quality
                h, w = bgr.shape[:2]
                maxdim = max(h, w)
                if maxdim <= limit:
                    return bgr
    ...
    def _draw_checkbox_checkmark(self, draw, x, y, w, h):
        ...
        try:
            ...
            if font_found and best_font and working_symbol:
                ...
                text_y = y + (h - text_h) / 2
                draw.text(
                    (text_x, text_y), working_symbol, fill="black", font=best_font
                )
                return
    ...
    def _draw_manual_checkmark(self, draw, x, y, w, h):
        ...
        try:
            ...
            draw.line(
                [(start_x, start_y), (mid_x, mid_y)], fill="black", width=line_width
            )
